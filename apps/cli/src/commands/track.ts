...
export function followUpDurationMs(flag: string | boolean | undefined): number | null {
...
export function selectedListSection(
  ...
  const sections = [opts.dueToday, opts.overdue, opts.upcoming, opts.recent, opts.all].filter(
    Boolean
  );
...
export function renderFollowUpLine(f: FollowUpRow, now: Date): string {
  ...
  const marker = overdue ? '⚠' : relativeDay(f.effectiveDueAt, now) === 'today' ? '→' : '·';
  const reason = f.reason ? ` · "${excerpt(f.reason, 40)}"` : '';
  const recurring = f.recurring && f.recurrenceRule ? ` · every ${f.recurrenceRule}` : '';
...
export function renderInteractionLine(i: InteractionWithContact, now: Date): string {
  const detail = excerpt(i.subject ?? i.content);
  const direction = i.direction ? ` ${i.direction === 'outbound' ? '→' : '←'}` : '';
  return `  ${utcDay(i.occurredAt)} (${relativeDay(i.occurredAt, now)}) · ${i.type}${direction} · ${i.contactName}${detail ? ` — "${detail}"` : ''}`;
}

function summaryLine(counts: { overdue: number; dueToday: number; upcoming: number }): string {
...
async function resolveContactId(
  ...
  return (await resolveContactRef(conn, selector)).id;
}
...
export async function executeTrackLog(
  ...
  const contact = await resolveContactRef(conn, selector);
  ...
  const channel = result.interaction.channel ? ` via ${result.interaction.channel}` : '';
  const when =
...
export async function executeTrackAdd(
  ...
  if (!metAt) {
    throw new Error('--met-at is required: where or how you met, e.g. --met-at "React Conf"');
  }
  const contact = await resolveContactRef(conn, selector);
  const followUpMs = followUpDurationMs(opts.followUp);
  ...
  if (followUpMs !== null) {
    followUp = await createFollowUp(
      conn,
      { contactId: contact.id, dueInMs: followUpMs, reason: opts.reason ?? `Follow up from ${metAt}` },
      { now }
    );
  }
...
export async function executeTrackList(
  ...
  const contactId = await resolveContactId(conn, opts.contact);

  if (section === 'recent') {
    const interactions = await listInteractions(conn, { contactId, limit });
  ...
  const summary = await listFollowUps(conn, { view, contactId, limit, now });
...
export async function executeTrackDone(
  ...
  const id = await resolveFollowUpId(conn, idOrPrefix);
  const result = await completeFollowUp(conn, id, { now });
...
export async function executeTrackSnooze(
  ...
  if (!forDuration && !until) {
    throw new Error('Snooze needs a target: --for 3d (relative) or --until 2026-10-01 (absolute).');
  }
  const input = until
  ...
  const snoozed = await snoozeFollowUp(conn, id, input, { now });
...
export async function executeTrackCancel(
  ...
  const id = await resolveFollowUpId(conn, idOrPrefix);
  const cancelled = await cancelFollowUp(conn, id, { now });
...
export function registerTrackCommand(program: Command): void {
  ...
  track
    .command('log <contact>')
    .description('Log an interaction with a contact (email, meeting, call, note…)')
    .option('--type <type>', 'Interaction type (default "note")', 'note')
    .option('--note <text>', 'What happened (stored as the interaction content)')
    .option('--subject <text>', 'Short subject line (e.g. the email subject)')
    .option('--channel <channel>', 'email | linkedin | twitter | in_person | phone | other')
    .option('--direction <dir>', 'inbound | outbound (defaults per type)')
    .option('--at <iso>', 'When it happened — ISO date/time; backdating allowed (default now)')
    .option('--follow-up [duration]', `Also schedule a follow-up (24h/7d/2w; default ${DEFAULT_FOLLOW_UP})`)
    .option('--reason <text>', 'Why the follow-up is needed')
    .option('--json', 'Print the result as JSON')
    .action((contact: string, opts: TrackLogOptions) =>
      run((conn) => executeTrackLog(contact, opts, conn))
    );

  track
    .command('add <contact>')
    .description('Record where you met someone and schedule the follow-up')
    .option('--met-at <text>', 'Where/how you met, e.g. "React Conf" (required)')
    .option('--at <iso>', 'When you met (default now; backdating allowed)')
    .option('--follow-up [duration]', `Follow-up window (24h/7d/2w; default ${DEFAULT_FOLLOW_UP} when the flag is given)`)
    .option('--reason <text>', 'Why the follow-up is needed')
    .option('--channel <channel>', 'Meeting channel (default in_person)')
    .option('--json', 'Print the result as JSON')
    .action((contact: string, opts: TrackAddOptions) =>
      run((conn) => executeTrackAdd(contact, opts, conn))
    );
  ...
  track
    ...
    .option('--contact <selector>', 'Filter to one contact (email, id, or full name)')
    .option('--limit <n>', 'Max rows (default 20, max 200)', '20')
    .option('--json', 'Print the result as JSON')
    .action((opts: TrackListOptions) => run((conn) => executeTrackList(opts, conn)));
  ...
  track
    ...
    .action((id: string, opts: TrackDoneOptions) =>
      run((conn) => executeTrackDone(id, opts, conn))
    );
  ...
  track
    ...
    .action((id: string, opts: TrackSnoozeOptions) =>
      run((conn) => executeTrackSnooze(id, opts, conn))
    );
  ...
  track
    ...
    .action((id: string, opts: { json?: boolean }) =>
      run((conn) => executeTrackCancel(id, opts, conn))
    );
}
