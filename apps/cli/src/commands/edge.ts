...
export async function executeEdgeAdd(
  ...
  const from = await resolveContactRef(conn, fromSel, scope);
  const to = await resolveContactRef(conn, toSel, scope);
...
export async function executeEdgeList(
  ...
  const contactId = opts.contact
    ? (await resolveContactRef(conn, opts.contact, scope)).id
    : undefined;
...
export async function executeEdgeRm(
  ...
  const id = await resolveEdgeId(conn, idOrPrefix, scope);
  const removed = await removeEdge(conn, id, { scope });
...
export async function executeEdgeImport(
  ...
  const summary = await importEdgesCsv(conn, csv, {
    now,
    relation: opts.relation,
    scope,
  });
  ...
  const lines = [
    `✓ Imported ${summary.imported} edges (${summary.merged} merged)`,
  ];
...
export async function executeEdgeMerge(
  ...
  const result = await mergeSymmetricPairs(conn, scope);
  if (opts.json) return JSON.stringify(result, null, 2);
...
export async function executeEdgeStatus(
  ...
  const id = await resolveEdgeId(conn, idOrPrefix, scope);
  const edge = await setEdgeStatus(conn, id, status, { now, scope });
...
export function registerEdgeCommand(program: Command): void {
  const edge = program
    .command("edge")
    .description("Manage graph edges between contacts");
  ...
  edge
    ...
    .option(
      "--relation <rel>",
      "mutual_network | colleague | met_at_event | mutual_intro | manual",
      "manual",
    )
    .option(
      "--source <src>",
      "linkedin_csv | manual | event_import | skype_migrate",
      "manual",
    )
    ...
    .option(
      "--at <iso>",
      "Discovered-at timestamp (unused; kept for flag parity)",
    )
    .option("--json", "Print the result as JSON")
    .action((from: string, to: string, opts: EdgeAddOptions) =>
      run(edge, (conn, scope) =>
        executeEdgeAdd(from, to, opts, conn, new Date(), scope),
      ),
    );
  ...
  edge
    ...
    .option("--json", "Print the result as JSON")
    .action((opts: EdgeListOptions) =>
      run(edge, (conn, scope) => executeEdgeList(opts, conn, scope)),
    );
  ...
  edge
    ...
    .action((id: string, opts: { json?: boolean }) =>
      run(edge, (conn, scope) => executeEdgeRm(id, opts, conn, scope)),
    );
  ...
  edge
    ...
    .action((csv: string, opts: { json?: boolean; relation?: string }) =>
      run(edge, (conn, scope) =>
        executeEdgeImport(csv, opts, conn, new Date(), scope),
      ),
    );
  ...
  edge
    ...
    .action((opts: { json?: boolean }) =>
      run(edge, (conn, scope) => executeEdgeMerge(opts, conn, scope)),
    );
  ...
  edge
    ...
    .action((id: string, opts: { json?: boolean }) =>
      run(edge, (conn, scope) =>
        executeEdgeStatus(id, "confirmed", opts, conn, new Date(), scope),
      ),
    );
  ...
  edge
    ...
    .action((id: string, opts: { json?: boolean }) =>
      run(edge, (conn, scope) =>
        executeEdgeStatus(id, "rejected", opts, conn, new Date(), scope),
      ),
    );
}
