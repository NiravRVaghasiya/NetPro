...
function when(startsAt: string | null, endsAt: string | null, now = new Date()): string {
  if (!startsAt) return 'no date';
  const day = utcDay(startsAt);
  const rel = relativeDay(startsAt, now);
  return endsAt && utcDay(endsAt) !== day ? `${day} → ${utcDay(endsAt)} (${rel})` : `${day} (${rel})`;
}
...
export function renderEventLine(e: EventSummary, now = new Date()): string {
  ...
  const people = e.attendeeCount === 1 ? '1 in your network' : `${e.attendeeCount} in your network`;
  return `  ${e.name}${where} · ${when(e.startsAt, e.endsAt, now)} · ${people}  [${e.id.slice(0, 8)}]`;
}

export function renderEventDetail(detail: EventDetail, now = new Date()): string {
  ...
  if (attendees.length === 0) {
    ...
    for (const a of attendees) {
      ...
      const meta = [
        ...
        a.relationshipScore !== null ? `score ${a.relationshipScore.toFixed(2)}` : null,
      ].filter(Boolean);
      lines.push(`    ${bits.join(' — ')}${meta.length > 0 ? ` (${meta.join(', ')})` : ''}`);
    }
  ...
  if (unmatched.length > 0) {
    lines.push(`  Unmatched attendees (${unmatched.length}) — link them by hand:`);
...
export function renderImportSummary(s: ImportEventsSummary): string {
  ...
  if (s.review > 0) lines.push(`  needs review: ${s.review} (re-run with --review to link them)`);
  if (s.ambiguous > 0) lines.push(`  ambiguous: ${s.ambiguous} — not linked, pick one by hand`);
  if (s.unmatched > 0) lines.push(`  unmatched: ${s.unmatched} — parked on the event for manual linking`);
  if (s.edges > 0) {
    lines.push(`  ${s.edges} met_at_event edge(s) written as pending — confirm them on /edges.`);
  }
  if (s.edgeCapReached) {
    lines.push('  ⚠ hit the per-event edge cap; remaining co-attendee links were skipped.');
  }
  ...
  for (const e of s.errors.slice(0, 5)) lines.push(`  row ${e.row}: ${e.reason}`);
  for (const w of s.warnings.slice(0, 5)) lines.push(`  row ${w.row}: ${w.reason}`);
  return lines.join('\n');
}
...
export function renderMatchResult(r: MatchEventResult): string {
  ...
  if (r.review > 0) lines.push(`  needs review: ${r.review}${r.applied ? '' : ' (add --review to include)'}`);
  if (r.ambiguous > 0) lines.push(`  ambiguous: ${r.ambiguous} — not linked`);
  if (r.unmatched > 0) lines.push(`  unmatched: ${r.unmatched}`);
  if (r.edges > 0) lines.push(`  ${r.edges} pending met_at_event edge(s) written.`);
  if (r.edgeCapReached) lines.push('  ⚠ hit the per-event edge cap; some links were skipped.');
  for (const m of r.matches.slice(0, 10)) {
    const who = m.ref.email ?? m.ref.name ?? '(blank)';
    lines.push(`    ${m.status.padEnd(9)} ${who}${m.reason ? ` — ${m.reason}` : ''}`);
  }
  return lines.join('\n');
}

export function renderRecommendations(rows: EventRecommendation[], now = new Date()): string {
  if (rows.length === 0) {
  ...
  for (const r of rows) {
    lines.push(`  ${r.score.toFixed(2)}  ${r.event.name} — ${when(r.event.startsAt, r.event.endsAt, now)}  [${r.event.id.slice(0, 8)}]`);
...
export async function executeEventsList(
  ...
  const lines = [`Events (${events.length} of ${total}):`, ...events.map((e) => renderEventLine(e, now))];
  const status = await eventsStatus(conn);
  lines.push(`  ${status.links} attendance row(s) across ${status.withAttendees} event(s).`);
  return lines.join('\n');
}
...
export async function executeEventsShow(
  ...
  const event = await resolveEventRef(conn, selector);
  const detail = await getEvent(conn, event.id);
  if (!detail) throw new EventError('not_found', `No event with id "${event.id}".`);
...
export async function executeEventsLink(
  ...
  const event = await resolveEventRef(conn, selector);
  const contact = await resolveContactRef(conn, contactSelector);
  const result = await linkAttendee(
    conn,
    { eventId: event.id, contactId: contact.id, role: opts.role ?? null, via: 'manual' },
    { now }
  );
...
export async function executeEventsUnlink(
  ...
  const event = await resolveEventRef(conn, selector);
  const contact = await resolveContactRef(conn, contactSelector);
  const result = await unlinkAttendee(conn, { eventId: event.id, contactId: contact.id });
  if (opts.json) return JSON.stringify({ event, contact, ...result }, null, 2);
...
export async function executeEventsRecommend(
  ...
  const rows = await recommendEvents(conn, { limit: parseLimit(opts.limit, 10, 50), now });
...
export async function executeEventsRm(
  ...
  if (opts.json) return JSON.stringify({ removed, remaining: await countEvents(conn) }, null, 2);
...
export function registerEventsCommand(program: Command): void {
  const events = program.command('events').description('Event matcher — who in your network went, and what to attend next (v2.0)');
  ...
  events
    ...
    .action((opts: EventsListOptions) => run((conn) => executeEventsList(opts, conn)));

  events
    .command('show <event>')
    .description('Show one event and its overlap with your network (id or exact name)')
    .option('--json', 'Print the result as JSON')
    .action((event: string, opts: { json?: boolean }) => run((conn) => executeEventsShow(event, opts, conn)));
  ...
  events
    ...
    .option('--json', 'Print the result as JSON')
    .action((name: string, opts: { location?: string; starts?: string; ends?: string; json?: boolean }) =>
      run((conn) => executeEventsAdd(name, opts, conn))
    );
  ...
  events
    ...
    .option('--no-edges', 'Do not create met_at_event edges between co-attendees')
    .option('--json', 'Print the summary as JSON')
    .action((csv: string, opts: EventsImportOptions) => run((conn) => executeEventsImport(csv, opts, conn)));

  events
    .command('match <event>')
    .description('Re-match an event’s unresolved attendees against the current network')
    .option('--apply', 'Write the links (default is a preview)')
    .option('--review', 'Also link last-name + initial matches')
    .option('--json', 'Print the result as JSON')
    .action((event: string, opts: EventsMatchOptions) => run((conn) => executeEventsMatch(event, opts, conn)));
  ...
  events
    ...
    .option('--json', 'Print the result as JSON')
    .action((event: string, contact: string, opts: { role?: string; json?: boolean }) =>
      run((conn) => executeEventsLink(event, contact, opts, conn))
    );
  ...
  events
    ...
    .action((event: string, contact: string, opts: { json?: boolean }) =>
      run((conn) => executeEventsUnlink(event, contact, opts, conn))
    );

  events
    .command('recommend')
    .description('Rank events by how many of your contacts went, industry fit and timing')
    .option('--limit <n>', 'Max rows (default 10)', '10')
    .option('--json', 'Print the result as JSON')
    .action((opts: { limit?: string; json?: boolean }) => run((conn) => executeEventsRecommend(opts, conn)));

  events
    .command('rm <event>')
    .description('Delete an event and its attendance rows')
    .option('--json', 'Print the result as JSON')
    .action((event: string, opts: { json?: boolean }) => run((conn) => executeEventsRm(event, opts, conn)));
}
