...
export function toSkillTarget(opts: SkillsGapOptions): SkillTarget {
  ...
  if (!target.role && !target.description && !target.skills) {
    throw new Error('Provide a target: --role "Staff Engineer", --description "<job text>" and/or --skills "python,k8s".');
  }
  return target;
}
...
export function parseExtractMode(value: string | undefined): ExtractMode {
  ...
  if (!(EXTRACT_MODES as readonly string[]).includes(mode)) {
    throw new Error(`Unknown --mode "${value}". Expected one of: ${EXTRACT_MODES.join(', ')}.`);
  }
...
function scoreTag(ref: { relationshipScore?: number | null }): string {
  return typeof ref.relationshipScore === 'number' ? ` · score ${ref.relationshipScore.toFixed(2)}` : '';
}
...
export function renderProfile(profile: SkillsProfile): string {
  ...
  const who = contact.email ? `${contact.fullName} <${contact.email}>` : contact.fullName;
  const lines = [`Skills for ${who}`];
  if (stored.length === 0) {
    lines.push('  Stored: none yet — run `netpro skills extract` to derive and store skills.');
  } else {
    lines.push(`  Stored (${stored.length}): ${stored.join(', ')}`);
  }
  if (current.details.length === 0) {
    lines.push('  Current text yields no taxonomy skills (headline, role, notes, tags and custom fields were scanned).');
  } else {
    ...
    for (const d of current.details) {
      ...
      lines.push(`    ${d.skill} [${d.category}] ${d.confidence.toFixed(2)}${where}`);
    }
  }
  if (unsupported.length > 0) {
    lines.push(`  Stored but not supported by the current text: ${unsupported.join(', ')}`);
  }
  const latest = profile.evidence[0];
  if (latest) {
    lines.push(`  Last extraction: ${latest.provider.replace('skills_', '')} at ${latest.fetchedAt.slice(0, 19).replace('T', ' ')}`);
  }
  return lines.join('\n');
}
...
export async function executeSkillsProfile(
  ...
  const ref = await resolveContactRef(conn, selector);
...
export function renderGap(result: GapResult, name: string): string {
  const lines = [`${name}: ${pct(result.matchScore)} match (${result.present.length}/${result.required.length} required)`];
  if (result.present.length) lines.push(`  Present: ${result.present.join(', ')}`);
...
export function renderNetworkGaps(analysis: NetworkGapAnalysis): string {
  ...
  if (analysis.required.length === 0) {
    lines.push('No taxonomy skills recognised in the target — try --skills with explicit names (see `netpro skills gap --help`).');
    if (analysis.target.unrecognized.length) {
      lines.push(`  Not in the taxonomy: ${analysis.target.unrecognized.join(', ')}`);
    }
    return lines.join('\n');
  }
  lines.push(
  ...
  if (analysis.target.unrecognized.length) {
    lines.push(`  (not in the taxonomy, ignored: ${analysis.target.unrecognized.join(', ')})`);
  }
  lines.push(`Network: ${analysis.contactCount} contact${analysis.contactCount === 1 ? '' : 's'} scanned · ${analysis.coveredCount}/${analysis.required.length} skills covered`);
  lines.push('');
  lines.push('Coverage:');
  for (const c of analysis.coverage) {
    const who = c.contacts.map((p) => `${p.fullName}${scoreTag(p)}`).join(', ');
    const more = c.count > c.contacts.length ? ` (+${c.count - c.contacts.length} more)` : '';
    const partial = c.partialCount ? ` · ${c.partialCount} partial` : '';
    lines.push(`  ${c.skill} [${skillCategory(c.skill)}] — ${c.count}${partial}${who ? `: ${who}${more}` : ''}`);
  }
  ...
  if (analysis.candidates.length) {
    ...
    for (const [i, c] of analysis.candidates.entries()) {
      ...
      const missing = c.gap.missing.length ? ` · missing ${c.gap.missing.join(', ')}` : '';
      lines.push(`  #${i + 1} ${c.fullName}${company} — ${pct(c.gap.matchScore)}${scoreTag(c)}${missing}`);
    }
  } else {
    lines.push('');
    lines.push('No contact matches any of the required skills yet — `netpro skills extract` derives skills from headlines, notes and tags.');
  }
  return lines.join('\n');
}

export async function executeSkillsGap(opts: SkillsGapOptions, conn: SqliteConn | PgConn): Promise<string> {
  ...
  if (selector) {
    ...
    const have = new Set<string>([...profile.current.skills, ...profile.stored]);
    const parsed = parseTarget(target);
    const gap = gapAnalysis(parsed, have);
    if (opts.json) return JSON.stringify({ contact: ref, target: parsed, gap }, null, 2);
  ...
  return opts.json ? JSON.stringify(analysis, null, 2) : renderNetworkGaps(analysis);
}
...
export function renderExtractSummary(summary: ExtractBatchSummary): string {
  ...
  for (const c of summary.changes.slice(0, 20)) {
    lines.push(`  ${c.fullName}: ${c.before.length ? c.before.join(', ') : '∅'} → ${c.after.length ? c.after.join(', ') : '∅'}`);
  }
  if (summary.changes.length > 20) lines.push(`  … ${summary.changes.length - 20} more (use --json for the full list)`);
  if (summary.aiErrors.length) {
    lines.push(`  AI pass failed for ${summary.aiErrors.length} contact${summary.aiErrors.length === 1 ? '' : 's'} (heuristics still applied): ${summary.aiErrors[0]!.error}`);
  }
  if (summary.indexed) lines.push(`  Search index refreshed for ${summary.indexed.indexed} contact${summary.indexed.indexed === 1 ? '' : 's'}.`);
  return lines.join('\n');
}
...
export async function executeSkillsExtract(
  ...
  const limit = opts.limit === undefined ? undefined : parseLimit(opts.limit, 10_000, 10_000);
  let contactIds: string[] | undefined;
  const selector = trim(opts.contact);
  if (selector) contactIds = [(await resolveContactRef(conn, selector)).id];
  ...
  return opts.json ? JSON.stringify(summary, null, 2) : renderExtractSummary(summary);
}

// ── skills status ────────────────────────────────────────────────────────

export async function executeSkillsStatus(opts: { json?: boolean }, conn: SqliteConn | PgConn): Promise<string> {
  const status = await skillsStatus(conn);
  ...
  if (status.neverExtracted > 0) lines.push('Run `netpro skills extract` to derive skills for every contact (offline, no key needed).');
  return lines.join('\n');
}
...
export function registerSkillsCommand(program: Command): void {
  const skills = program
    .command('skills')
    .description('Skills gap analyzer — derive skills, compare a target role against your network (v2.0)')
    .argument('[contact]', 'Show stored + derived skills for one contact (name, email or id)')
    .option('--json', 'Print the result as JSON')
    .action((contact: string | undefined, opts: { json?: boolean }, cmd: Command) => {
      if (!contact) {
        cmd.help();
        return;
      }
      return run((conn) => executeSkillsProfile(contact, opts, conn));
    });

  skills
    .command('gap')
    .description('Who in your network covers a target role / description / skill list')
    .option('--role <role>', 'Target role, e.g. "Staff Data Engineer"')
    .option('--description <text>', 'Job description or requirement list')
    .option('--skills <list>', 'Explicit skills, comma-separated (e.g. "python,k8s,aws")')
    .option('--contact <selector>', 'Compare one contact against the target instead of the whole network')
    .option('--limit <n>', 'Ranked candidates to show (default 10, max 100)')
    .option('--json', 'Print the analysis as JSON')
    .action((opts: SkillsGapOptions) => run((conn) => executeSkillsGap(opts, conn)));

  skills
    .command('extract')
    .description('Derive skills from headline, role, notes, tags and custom fields, and store them')
    .option('--mode <mode>', 'heuristic (default, offline) | ai (adds a BYO-key model pass)')
    .option('--contact <selector>', 'Only this contact')
    .option('--limit <n>', 'Stop after N contacts')
    .option('--provider <provider>', 'openai | anthropic override for --mode ai')
    .option('--dry-run', 'Show what would change without writing anything')
    .option('--json', 'Print the summary as JSON')
    .action((opts: SkillsExtractOptions) => run((conn) => executeSkillsExtract(opts, conn)));

  skills
    .command('status')
    .description('How many contacts have derived skills')
    .option('--json', 'Print the counts as JSON')
    .action((opts: { json?: boolean }) => run((conn) => executeSkillsStatus(opts, conn)));
}
