...
vi.mock('../../src/config/keychain', () => ({
  Keychain: {
    get: vi.fn(async (key: string) => (key === 'user.name' ? 'Alex Keychain' : null)),
  },
}));
...
function stubOpenAiFetch(): ReturnType<typeof vi.fn> {
  const fn = vi.fn(async () =>
    ({
      ok: true,
      status: 200,
      json: async () => ({ choices: [{ message: { content: JSON.stringify({ subject: 'Hi Jane', body: 'Loved your work at Stripe.' }) } }] }),
    }) as Response,
  );
...
describe('toOutreachInput', () => {
  ...
  it('maps ad-hoc recipient flags', () => {
    const input = toOutreachInput({ name: 'Pat Lee', email: 'pat@x.com', company: 'X', tone: 'warm' });
    expect(input.selector).toBeUndefined();
    expect(input.adHoc).toEqual({ name: 'Pat Lee', email: 'pat@x.com', company: 'X', role: undefined });
    expect(input.compose.tone).toBe('warm');
  });

  it('rejects an unknown tone', () => {
    expect(() => toOutreachInput({ to: 'x@y.com', tone: 'shouty' })).toThrow(/Unknown --tone/);
  });

  it('rejects mixing --to with ad-hoc recipient flags', () => {
    expect(() => toOutreachInput({ to: 'x@y.com', name: 'Pat' })).toThrow(/cannot be combined/);
  });

  it('requires any recipient at all', () => {
    expect(() => toOutreachInput({ context: 'hi' })).toThrow(/Provide a recipient/);
  });
});
...
describe('executeOutreach', () => {
  ...
  it('fails clearly when no AI key is configured', async () => {
    delete process.env.OPENAI_API_KEY;
    await expect(executeOutreach({ to: 'jane@stripe.com' }, conn)).rejects.toThrow(
      /ai\.openai\.key/,
    );
  });

  it('surfaces contact resolution errors (unknown / ambiguous)', async () => {
    await expect(executeOutreach({ to: 'nobody@nowhere.com' }, conn)).rejects.toThrow(
      /No contact matches/,
    );
  });
});
