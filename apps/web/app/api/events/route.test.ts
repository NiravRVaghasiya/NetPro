...
function postCsv(csv: string, qs = ""): Promise<Response> {
  ...
  return POST(
    new Request(`http://localhost/api/events${qs}`, {
      method: "POST",
      body: form,
    }),
  );
...
describe("GET /api/events", () => {
  ...
  it("filters and paginates", async () => {
    for (const name of ["React Conf", "RustConf", "PyCon"])
      await postJson({ name });
...
describe("POST /api/events — CSV import", () => {
  ...
  it("imports, matches attendees and reports the unmatched bucket", async () => {
    ...
    expect(body.unmatchedRefs).toEqual([
      { name: null, email: "nobody@nowhere.dev", reason: expect.any(String) },
    ]);
  });
  ...
  it("reports ambiguous lines with candidates and links nothing", async () => {
    fixture.conn.db
      ...
      .run();
    const body = (await (
      await postCsv("name,names\nMeetup,Ada Lovelace")
    ).json()) as ImportBody;
    expect(body.ambiguous).toBe(1);
    expect(body.attendees).toBe(0);
    expect(body.ambiguousRefs[0]!.candidates.map((c) => c.id)).toEqual([
      "a",
      "a2",
    ]);
  });
