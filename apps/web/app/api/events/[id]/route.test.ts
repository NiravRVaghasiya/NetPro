...
async function link(eventId: string, contactId: string): Promise<void> {
  ...
  await linkAttendee(
    fixture.conn,
    { eventId, contactId, via: "manual", edges: false },
    { now: new Date(NOW) },
  );
}

const get = (id: string) =>
  GET(new Request(`http://localhost/api/events/${id}`), {
...
describe("DELETE /api/events/[id]", () => {
  it("removes the event and its attendance rows", async () => {
    ...
    expect(res.status).toBe(200);
    expect(
      ((await res.json()) as { removed: { name: string } }).removed.name,
    ).toBe("React Conf");
