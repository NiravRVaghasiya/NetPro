...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    const ref = await resolveOptionalContact(conn, sp.get("contact"), scope);
    if (ref) {
      const profile = await getSkillsProfile(conn, ref.id, scope);
      if (!profile)
        return crmJson({ error: `No contact found with id "${ref.id}".` }, 404);
      ...
      const gap = gapAnalysis(
        parsed,
        new Set<string>([...profile.current.skills, ...profile.stored]),
      );
      return crmJson({
        contact: {
          id: ref.id,
