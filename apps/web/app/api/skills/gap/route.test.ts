...
const get = (qs: string) =>
  GET(new Request(`http://localhost/api/skills/gap${qs}`));
...
describe("GET /api/skills/gap", () => {
  it("rejects an empty target with 400", async () => {
    ...
    expect(await res.json()).toEqual({
      error: "Provide at least one of role, description or skills.",
    });
  ...
  it("analyses the live network for a role + explicit skills", async () => {
    ...
    expect(python.contacts.map((c) => c.id)).toEqual([
      "a",
      "b",
      "dup1",
      "dup2",
  ...
  it("reports unrecognised names and honours limit / perSkill bounds", async () => {
    const body = (await (
      await get("?skills=python,cobol&limit=1&perSkill=2")
    ).json()) as NetworkBody;
    ...
    const loose = (await (
      await get("?skills=python&limit=abc&perSkill=-4")
    ).json()) as NetworkBody;
  ...
  it("returns an empty-but-honest analysis when nothing in the target is a skill", async () => {
    const body = (await (
      await get("?description=a%20kind%20person")
    ).json()) as NetworkBody;
    expect(body.required).toEqual([]);
  ...
  it("compares one contact when ?contact= is given (name, email or id)", async () => {
    const res = await get(
      "?contact=ada%40engines.dev&skills=python,rust,javascript",
    );
    expect(res.status).toBe(200);
  ...
  it("answers unknown → 404 and ambiguous → 400 through the shared resolver", async () => {
    ...
    expect((await ambiguous.json()).error).toMatch(
      /Ambiguous contact "Sam Same"/,
    );
