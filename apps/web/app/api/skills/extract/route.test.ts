...
vi.mock("@netpro/core/src/ai", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveAiProvider: (credentials: {
      openaiKey?: string | null;
      anthropicKey?: string | null;
    }) => {
      if (!credentials.openaiKey && !credentials.anthropicKey) {
        throw new actual.AiProviderError(
          "not_configured",
...
beforeEach(() => {
  fixture.sqlite.exec(
    "DELETE FROM enrichments; DELETE FROM activity_log; DELETE FROM search_index; DELETE FROM contacts;",
  );
  const rows = [
    {
      id: "a",
...
describe("POST /api/skills/extract", () => {
  it("extracts heuristically for every live contact with an empty body, and is idempotent", async () => {
    ...
    expect(
      body.changes.find((c: { contactId: string }) => c.contactId === "a")
        .after,
    ).toEqual(["python", "aws", "kubernetes"]);
    expect(body.indexed).toEqual({ indexed: 3, skipped: 0 });
    expect(
      fixture.sqlite
        .prepare(
          "SELECT count(*) AS n FROM enrichments WHERE provider = 'skills_heuristic'",
        )
        .get(),
    ).toEqual({ n: 3 });
    expect(
      fixture.sqlite
        .prepare(
          "SELECT count(*) AS n FROM activity_log WHERE action = 'skills.extracted'",
        )
        .get(),
    ).toEqual({ n: 3 });
  ...
  it("previews with dryRun and scopes to one contact", async () => {
    const dry = await (
      await post({ dryRun: true, contact: "Bob Bridge" })
    ).json();
    expect(dry).toMatchObject({ scanned: 1, updated: 1, dryRun: true });
    expect(dry.changes[0]).toMatchObject({ contactId: "b", before: [] });
    expect(dry.changes[0].after).toEqual(
      expect.arrayContaining(["sql", "dbt", "data engineering"]),
    );
    expect(
      fixture.sqlite.prepare("SELECT count(*) AS n FROM enrichments").get(),
    ).toEqual({ n: 0 });
    expect(
      fixture.sqlite
        .prepare("SELECT skills FROM contacts WHERE id = 'b'")
        .get(),
    ).toEqual({ skills: null });
  });
  ...
  it("runs the AI pass when a key is configured, keeping the model inside the taxonomy", async () => {
    ...
    expect(body.changes[0].after).toEqual([
      "typescript",
      "python",
      "aws",
      "kubernetes",
    ...
    const row = fixture.sqlite
      .prepare(
        "SELECT provider, raw_payload FROM enrichments WHERE contact_id = 'a'",
      )
      .all() as Array<{
      provider: string;
      raw_payload: string;
    ...
    const payload = JSON.parse(row[0]!.raw_payload) as {
      details: Array<{ skill: string; source: string }>;
    };
    expect(payload.details.find((d) => d.skill === "typescript")).toMatchObject(
      { source: "ai" },
    );
    expect(payload.details.find((d) => d.skill === "python")).toMatchObject({
      source: "heuristic",
    });
  });
  ...
  it("degrades to the heuristic result when the model call fails, and says so", async () => {
    ...
    expect(body.aiErrors).toEqual([
      {
        contactId: "a",
        error:
          "upstream_error: AI provider request failed. Check credentials and provider availability.",
      },
    ]);
