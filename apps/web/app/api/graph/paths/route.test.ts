...
describe("GET /api/graph/paths", () => {
  it("plans the confirmed chain with ranked paths and the ask", async () => {
    const res = await get(
      "/api/graph/paths?target=Zoe%20Target&from=ada@engines.dev",
    );
    ...
    expect(body.paths[0]!.path.map((n) => n.contactId)).toEqual([
      "a",
      "b",
      "z",
    ]);
  ...
  it("caps depth at 6 (web budget) — 99 falls back to 6, banana to the default 4", async () => {
    const capped = (await (
      await get("/api/graph/paths?target=z&from=a&depth=99")
    ).json()) as { maxDepth: number };
    expect(capped.maxDepth).toBe(6);
    const def = (await (
      await get("/api/graph/paths?target=z&from=a&depth=banana")
    ).json()) as { maxDepth: number };
    expect(def.maxDepth).toBe(4);
    const zero = (await (
      await get("/api/graph/paths?target=z&from=a&depth=0")
    ).json()) as { maxDepth: number };
