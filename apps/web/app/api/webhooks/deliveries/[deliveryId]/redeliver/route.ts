...
import { redeliverWebhook, WebhookError } from '@netpro/core/src/webhooks';

export async function POST(_req: Request, { params }: { params: Promise<{ deliveryId: string }> }): Promise<Response> {
  const { deliveryId } = await params;
  try {
    const scope = await requireScope('admin');
    const result = await redeliverWebhook(conn, deliveryId, scope);
