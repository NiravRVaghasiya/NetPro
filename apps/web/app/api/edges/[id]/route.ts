...
export async function PATCH(
  ...
  try {
    ...
    const id = await resolveEdgeId(conn, raw, scope);
    ...
    if (
      ...
      return crmJson(
        {
          error: "action must be confirm/confirmed or reject/rejected.",
          code: "invalid_input",
        },
        400,
      );
    ...
    const edge = await setEdgeStatus(conn, id, status, { scope });
    return crmJson({ edge });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    const { id: raw } = await context.params;
    const id = await resolveEdgeId(conn, raw, scope);
    const edge = await removeEdge(conn, id, { scope });
    return crmJson({ edge });
  } catch (error) {
    return crmErrorResponse(error);
  }
