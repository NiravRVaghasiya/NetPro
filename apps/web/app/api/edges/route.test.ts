...
beforeEach(() => {
  fixture.sqlite.exec(
    "DELETE FROM event_attendees; DELETE FROM events; DELETE FROM edges; DELETE FROM contacts;",
  );
  seed();
});
...
describe("POST /api/edges", () => {
  it("creates a confirmed manual edge and returns 201", async () => {
    const res = await POST(
      post({ sourceId: "c1", targetId: "c2", relation: "colleague" }),
    );
    expect(res.status).toBe(201);
    const body = (await res.json()) as {
      edge: { relation: string; status: string };
    };
  ...
  it("maps validation failures to 400 and unknown contacts to 404", async () => {
    expect((await POST(post({ sourceId: "c1", targetId: "c1" }))).status).toBe(
      400,
    );
    expect(
      (await POST(post({ sourceId: "c1", targetId: "nobody" }))).status,
    ).toBe(404);
    const unknownRel = await POST(
      post({ sourceId: "c1", targetId: "c2", relation: "besties" }),
    );
    expect(unknownRel.status).toBe(400);
    expect(((await unknownRel.json()) as { code: string }).code).toBe(
      "invalid_input",
    );
  });
...
describe("/api/edges/[id]", () => {
  it("confirms and deletes", async () => {
    ...
    expect(
      ((await patched.json()) as { edge: { status: string } }).edge.status,
    ).toBe("confirmed");
  ...
  it("maps unknown ids to 404", async () => {
    const res = await DELETE(
      new Request("http://localhost/api/edges/missing"),
      {
