...
describe('POST /api/campaigns', () => {
  it('creates a draft and snapshots recipients (201)', async () => {
    ...
    const body = (await res.json()) as {
      campaign: { id: string; status: string; type: string; totalRecipients: number };
      added: number;
    };
  ...
  it('maps validation errors to 400 with a code', async () => {
    const res = await POST(post('http://localhost/api/campaigns', { name: '', template }));
  ...
  it('rejects a non-JSON body with 415', async () => {
    const res = await POST(
      new Request('http://localhost/api/campaigns', { method: 'POST', body: 'name=x' })
    );
...
describe('GET /api/campaigns', () => {
  it('lists campaigns and filters by status', async () => {
    ...
    const b = await POST(post('http://localhost/api/campaigns', { name: 'B', template }));
    const bId = ((await b.json()) as { campaign: { id: string } }).campaign.id;
    await PATCH(patch(`http://localhost/api/campaigns/${bId}`, { action: 'status', status: 'active' }), {
    ...
    const active = await GET(new Request('http://localhost/api/campaigns?status=active'));
    const activeBody = (await active.json()) as { campaigns: { name: string }[] };
    expect(activeBody.campaigns.map((c) => c.name)).toEqual(['B']);

    const bad = await GET(new Request('http://localhost/api/campaigns?status=launched'));
...
describe('GET /api/campaigns/[id]', () => {
  it('renders recipients with personalized drafts', async () => {
    ...
    const res = await GetById(new Request(`http://localhost/api/campaigns/${id}`), {
      params: Promise.resolve({ id }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      campaign: { name: string };
      recipients: { contactName: string; draft: { subject: string; body: string } | null }[];
  ...
  it('404s for unknown campaigns', async () => {
    const res = await GetById(new Request('http://localhost/api/campaigns/nope'), {
...
describe('PATCH /api/campaigns/[id]', () => {
  async function makeDraft() {
    const created = await POST(post('http://localhost/api/campaigns', { name: 'P', template }));
  ...
  it('walks the lifecycle via action=status', async () => {
    ...
    const activate = await PATCH(
      patch(`http://localhost/api/campaigns/${id}`, { action: 'status', status: 'active' }),
      { params: Promise.resolve({ id }) }
    );
  ...
  it('rejects an illegal transition with 409', async () => {
    ...
    const res = await PATCH(
      patch(`http://localhost/api/campaigns/${id}`, { action: 'status', status: 'completed' }),
      { params: Promise.resolve({ id }) }
    );
  ...
  it('rejects an unknown status with 400', async () => {
    const id = await makeDraft();
    const res = await PATCH(
      patch(`http://localhost/api/campaigns/${id}`, { action: 'status', status: 'launched' }),
      { params: Promise.resolve({ id }) }
    );
    expect(res.status).toBe(400);
  });
  ...
  it('edits a draft via action=update and freezes once active', async () => {
    ...
    await PATCH(patch(`http://localhost/api/campaigns/${id}`, { action: 'status', status: 'active' }), {
      params: Promise.resolve({ id }),
    });
    const frozen = await PATCH(
      patch(`http://localhost/api/campaigns/${id}`, { action: 'update', name: 'Nope' }),
      { params: Promise.resolve({ id }) }
    );
  ...
  it('rejects an unknown action with 400', async () => {
    const id = await makeDraft();
    const res = await PATCH(patch(`http://localhost/api/campaigns/${id}`, { action: 'launch' }), {
      params: Promise.resolve({ id }),
    });
    expect(res.status).toBe(400);
  });
...
describe('POST /api/campaigns/[id]/recipients/[recipientId]', () => {
  async function activeCampaignWith(recipientContactIds: string[], opts: { dailyLimit?: number; steps?: unknown[] } = {}) {
    const created = await POST(
      post('http://localhost/api/campaigns', {
        name: 'M',
        template,
    ...
    const render = await GetById(new Request(`http://localhost/api/campaigns/${id}`), {
      params: Promise.resolve({ id }),
    });
    const recipients = ((await render.json()) as { recipients: { id: string; contactId: string }[] }).recipients;
    return { id, recipients };
  }

  function mark(id: string, recipientId: string, body: unknown) {
    return MarkRecipient(
      post(`http://localhost/api/campaigns/${id}/recipients/${recipientId}`, body),
  ...
  it('marks sent: logs an interaction and returns the updated recipient', async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { recipient: { status: string }; interactionId: string; campaign: { sent: number } };
  ...
  it('requires an active campaign (409 on a draft)', async () => {
    const created = await POST(
      post('http://localhost/api/campaigns', { name: 'D', template, recipients: { contactIds: ['c1'] } })
    );
    const id = ((await created.json()) as { campaign: { id: string } }).campaign.id;
    const render = await GetById(new Request(`http://localhost/api/campaigns/${id}`), {
      params: Promise.resolve({ id }),
    });
    const recipientId = ((await render.json()) as { recipients: { id: string }[] }).recipients[0]!.id;
    const res = await mark(id, recipientId, { action: 'sent' });
    expect(res.status).toBe(409);
  });

  it('enforces the daily limit (409) and honors force', async () => {
    const { id, recipients } = await activeCampaignWith(['c1', 'c2'], { dailyLimit: 1 });
  ...
  it('schedules the next drip step for a sequence', async () => {
    ...
    const recipientId = recipients.find((r) => r.contactId === 'c1')!.id;
    const res = await mark(id, recipientId, { action: 'sent' });
    const body = (await res.json()) as { recipient: { status: string; scheduledAt: string | null; draft: { subject: string } | null } };
  ...
  it('records a reply and cancels the drip', async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { recipient: { status: string }; campaign: { replied: number } };
  ...
  it('skips without logging an interaction', async () => {
    ...
    expect(res.status).toBe(200);
    expect(((await res.json()) as { recipient: { status: string } }).recipient.status).toBe('skipped');
