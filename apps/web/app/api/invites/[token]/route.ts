...
export async function GET(
  ...
  try {
    ...
    if (!invite)
      return crmJson({ error: "Invite not found", valid: false }, 404);
    if (invite.revokedAt)
      return crmJson({ error: "Invite revoked", valid: false }, 410);
    if (invite.acceptedAt)
      return crmJson({ error: "Invite already accepted", valid: false }, 410);
    if (isInviteExpired(invite.expiresAt))
      return crmJson({ error: "Invite expired", valid: false }, 410);
