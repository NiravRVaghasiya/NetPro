...
export default async function EdgesPage({
  ...
  const [edges, total, pending] = await Promise.all([
    listEdges(conn, { status, relation, limit: 100 }),
    countEdges(conn, { status, relation }),
    countEdges(conn, { status: 'pending' }),
  ]);
  ...
  return (
    <div>
      ...
      <p style={{ color: '#475569' }}>
        Graph links between your contacts. Inferred mutuals stay <strong>pending</strong> until you
        confirm them — NetPro never silently claims two people know each other.
      </p>
      <p>
      ...
      {edges.length === 0 ? (
        <p>
          No edges yet. Add a manual link below, or import a LinkedIn CSV that includes a Mutual
          connections column — candidates appear here for confirmation.
        </p>
      ) : (
