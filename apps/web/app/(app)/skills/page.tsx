...
function Coverage({ analysis }: { analysis: NetworkGapAnalysis }) {
  return (
    <section style={{ marginTop: "1.5rem" }}>
      <h2>
        Coverage — {analysis.coveredCount}/{analysis.required.length} skills
        covered by {analysis.contactCount} contact
        {analysis.contactCount === 1 ? "" : "s"}
      </h2>
      {analysis.gaps.length > 0 ? (
        <p style={{ color: "#b45309" }}>
          Nobody in your network covers:{" "}
          <strong>{analysis.gaps.join(", ")}</strong>.
        </p>
      ) : (
        <p style={{ color: "#15803d" }}>
          Every required skill is covered by at least one contact.
        </p>
      )}
              ...
              <td style={{ padding: "0.4rem 0.5rem" }}>
                {c.contacts.length === 0 ? (
                  ...
                  c.contacts.map((p, i) => (
                    <span key={p.id}>
                      {i > 0 ? ", " : ""}
                      <Link href={`/contacts/${encodeURIComponent(p.id)}`}>
                        {p.fullName}
                      </Link>
                      <span style={{ color: "#6b7280", fontSize: "0.8rem" }}>
                        {" "}
                        ({scoreLabel(p.relationshipScore)})
                      </span>
                    </span>
                  ))
                )}
  ...
  if (analysis.candidates.length === 0) {
    return (
      <p style={{ color: "#9ca3af", marginTop: "1.5rem" }}>
        No contact matches any of the required skills yet. Derive skills below,
        or add notes and tags to the people who have them.
      </p>
    );
  }
  return (
      ...
      <ol style={{ paddingLeft: "1.25rem" }}>
        {analysis.candidates.map((c) => (
          <li key={c.id} style={{ marginBottom: "0.5rem" }}>
            <Link href={`/contacts/${encodeURIComponent(c.id)}`}>
              {c.fullName}
            </Link>
            {c.company ? (
              <span style={{ color: "#6b7280" }}> · {c.company}</span>
            ) : null}
            {c.role ? (
              <span style={{ color: "#6b7280" }}> · {c.role}</span>
            ) : null}{" "}
            — <strong>{pct(c.gap.matchScore)}</strong> match · tie{" "}
            {scoreLabel(c.relationshipScore)}
            <div style={{ fontSize: "0.85rem", color: "#374151" }}>
              {c.gap.present.length > 0 ? (
                <span>has {c.gap.present.join(", ")}</span>
              ) : null}
              ...
              {c.gap.missing.length > 0 ? (
                <span style={{ color: "#6b7280" }}>
                  {c.gap.present.length + c.gap.partial.length > 0 ? " · " : ""}
                  missing {c.gap.missing.join(", ")}
                </span>
              ) : null}
              {" · "}
              <Link href={`/graph?target=${encodeURIComponent(c.id)}`}>
                Find a warm intro
              </Link>
            </div>
          </li>
...
export default async function SkillsPage({
  ...
  const [status, contacts] = await Promise.all([
    skillsStatus(conn, scope),
    loadSkillContacts(conn, { scope }),
  ]);
  ...
  return (
    <div>
      ...
      <p style={{ color: "#475569" }}>
        Describe a role and NetPro shows which required skills your network
        covers, who has them, and where the gaps are. Skills are derived from
        headlines, notes and tags against a fixed, explainable taxonomy —
        nothing is guessed. <Link href="/graph">Warm intros</Link> ·{" "}
      ...
      >
        ...
        <label style={{ display: "grid", gap: "0.25rem" }}>
          <span style={{ fontSize: "0.8rem", color: "#6b7280" }}>
            Job description or requirements (optional)
          </span>
          <textarea
            name="description"
            ...
            rows={4}
            placeholder={
              "Own our data platform: Python, dbt and Airflow on AWS.\nComfortable with Kubernetes."
            }
        ...
        <label style={{ display: "grid", gap: "0.25rem" }}>
          <span style={{ fontSize: "0.8rem", color: "#6b7280" }}>
            Explicit skills (comma-separated, optional)
          </span>
          <input
            name="skills"
            defaultValue={skills ?? ""}
            placeholder="python, k8s, aws"
            style={{ padding: "0.4rem" }}
          />
        </label>
        <div>
        ...
        analysis.required.length === 0 ? (
          ...
          <>
            <p>
              ...
              {analysis.required.length === 1 ? "" : "s"}:{" "}
              {analysis.required.join(", ")}
              {analysis.target.unrecognized.length > 0 ? (
                <span style={{ color: "#6b7280" }}>
                  {" "}
                  (ignored, not in the taxonomy:{" "}
                  {analysis.target.unrecognized.join(", ")})
                </span>
              ) : null}
            </p>
      ...
      <ExtractPanel
        neverExtracted={status.neverExtracted}
          ...
          {map.counts.length === 0 ? (
            ...
            <>
              <p style={{ color: "#6b7280" }}>
                {map.withSkills} of {map.contactCount} contacts have at least
                one recognised skill. Top skills:
              </p>
              <ul
                style={{
                  display: "flex",
