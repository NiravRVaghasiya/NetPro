...
vi.mock('./panels', () => ({
  ExtractPanel: ({ neverExtracted, total }: { neverExtracted: number; total: number }) => (
...
function seed(): void {
  ...
  const rows = [
    { id: 'a', fullName: 'Ada Lovelace', company: 'Engines', headline: 'Staff engineer — Python, Kubernetes, AWS', relationshipScore: 0.9 },
    { id: 'b', fullName: 'Bob Bridge', role: 'Data Engineer', notes: 'SQL and dbt.', tags: ['python'], relationshipScore: 0.6 },
...
describe('/skills page', () => {
  it('renders the network skill map and the extraction panel with no target', async () => {
    ...
    expect(html).toContain('3 of 3 contacts have at least one recognised skill');
  ...
  it('analyses a target: required skills, coverage table, gaps and ranked matches', async () => {
    const html = await render({ role: 'Data Engineer', skills: 'python, k8s, swift' });
    ...
    expect(html).toContain('Nobody in your network covers: <strong>swift</strong>');
  ...
  it('handles an empty database', async () => {
    ...
    expect(analysed).toContain('No contact matches any of the required skills yet');
  });
});
