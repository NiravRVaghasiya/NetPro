...
export default async function ContactDetailPage({
  ...
  const skills = await getSkillsProfile(conn, id, scope);
  // v2.0 Phase 6 — where you crossed paths with this person.
  const events = await listContactEvents(conn, id, scope);
  // v2.5 Phase 5 — content this person is part of (co-authored, mentioned…).
  const content = await listContactContent(conn, id, scope);
  ...
  return (
    <div>
      ...
      {contact.headline && (
        <p style={{ margin: "0.25rem 0" }}>{contact.headline}</p>
      )}
      ...
      <p style={{ marginTop: "0.75rem" }}>
        ...
        {stats.interactionCount} interaction
        {stats.interactionCount === 1 ? "" : "s"} · last touch{" "}
        {stats.lastInteraction
          ? relativeDayLabel(stats.lastInteraction, now)
          : "never"}
      </p>
      ...
      <section aria-label="Pending follow-ups" style={{ marginTop: "1.25rem" }}>
        <h2 style={{ fontSize: "1rem" }}>
          Follow-ups
          {followUps.length > 0 ? ` (${followUps.length} pending)` : ""}
        </h2>
        {followUps.length === 0 ? (
      ...
      <section aria-label="Interaction history" style={{ marginTop: "1.5rem" }}>
        ...
        {interactions.length === 0 ? (
          <p style={{ margin: "0.25rem 0" }}>
            No interactions yet — log the first one below. Everything you log
            feeds the relationship score and the network analytics.
          </p>
        ) : (
          <ul style={{ margin: "0.25rem 0", paddingLeft: "1.25rem" }}>
            {interactions.map((i) => (
      ...
      <section aria-label="Events" style={{ marginTop: "1.5rem" }}>
        ...
        {events.length === 0 ? (
          <p style={{ margin: "0.25rem 0", color: "#9ca3af" }}>
            No events recorded — record where you met below, or import an
            attendee list.
          </p>
        ) : (
          <ul style={{ margin: "0.25rem 0", paddingLeft: "1.25rem" }}>
            {events.map((e) => (
              <li key={e.id} style={{ marginBottom: "0.25rem" }}>
                ...
                {e.location ? (
                  <span style={{ color: "#6b7280" }}> · {e.location}</span>
                ) : null}
                {e.startsAt ? (
      ...
      <section aria-label="Content" style={{ marginTop: "1.5rem" }}>
        ...
        {content.length === 0 ? (
          <p style={{ margin: "0.25rem 0", color: "#9ca3af" }}>
            No content links this person yet — mention them on a piece&apos;s
            page (co-authored, quoted, reviewed…).
          </p>
        ) : (
          <ul style={{ margin: "0.25rem 0", paddingLeft: "1.25rem" }}>
            {content.map((c) => (
              <li key={c.id} style={{ marginBottom: "0.25rem" }}>
                <Link href={`/content/${encodeURIComponent(c.id)}`}>
                  {c.title}
                </Link>
                <span style={{ color: "#6b7280" }}>
                  {" "}
...
function SkillsSection({
  ...
  profile: NonNullable<Awaited<ReturnType<typeof getSkillsProfile>>>;
}) {
    ...
    <section aria-label="Skills" style={{ marginTop: "1.25rem" }}>
      ...
      {tags.length === 0 ? (
        <p style={{ color: "#9ca3af" }}>
          No taxonomy skills recognised in this contact&apos;s headline, role,
          notes or tags yet.
        </p>
      ) : (
        <ul
      ...
      {current.details.length > 0 ? (
        <details style={{ marginTop: "0.5rem" }}>
          <summary
            style={{ cursor: "pointer", color: "#6b7280", fontSize: "0.85rem" }}
          >
          ...
          >
            {current.details.map((d) => (
              <li key={d.skill}>
                <strong>{d.skill}</strong> [{d.category}] ·{" "}
        ...
        {stored.length === 0
          ...
          : `Stored ${latest ? `by ${latest.provider.replace("skills_", "")} extraction on ${utcDay(latest.fetchedAt)}` : ""}${
              unsupported.length > 0
                ? ` · ${unsupported.join(", ")} stored but not supported by the current text`
                : ""
            }.`}
      </p>
    </section>
