...
async function render(
...
): Promise<{ html: string; props: Record<string, unknown> }> {
  ...
  return {
    html,
    props: raw
      ? (JSON.parse(raw.replace(/&quot;/g, '"')) as Record<string, unknown>)
      : {},
  };
}
...
describe("/outreach deep-link prefill (v2.0 Phase 3)", () => {
  ...
  it("resolves contactId into an initial contact and forwards context/purpose", async () => {
    ...
    expect(props.initialContact).toMatchObject({
      id: "a",
      fullName: "Ada Lovelace",
      email: "ada@engines.dev",
    });
  ...
  it("ignores unknown or deleted contact ids instead of crashing", async () => {
    ...
    fixture.sqlite.exec(
      "UPDATE contacts SET deleted_at = '2026-09-06T00:00:00.000Z';",
    );
