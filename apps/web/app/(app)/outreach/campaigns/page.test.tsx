...
async function render(searchParams: Record<string, string> = {}): Promise<string> {
  const element = await CampaignsPage({ searchParams: Promise.resolve(searchParams) });
...
describe('/outreach/campaigns list page', () => {
  ...
  it('lists campaigns with status, type, and counts', async () => {
    ...
    await createCampaign(fixture.conn, { name: 'Newsletter', template }, { now: NOW });
    await setCampaignStatus(fixture.conn, draft.campaign.id, 'active', { now: NOW });
  ...
  it('filters by status from the query string', async () => {
    const draft = await createCampaign(fixture.conn, { name: 'Reactivation', template }, { now: NOW });
    await createCampaign(fixture.conn, { name: 'Newsletter', template }, { now: NOW });
    await setCampaignStatus(fixture.conn, draft.campaign.id, 'active', { now: NOW });
