...
export default async function CampaignDetailPage({
  ...
  const render = await renderCampaign(conn, id, {}, scope);
  if (!render) notFound();
    ...
    <div>
      ...
      <p style={{ margin: "0.25rem 0" }}>
        ...
        {campaign.type === "sequence"
          ? `${sequenceLength}-message sequence`
          : "single message"}{" "}
        · daily limit {campaign.dailyLimit}
        {campaign.sendFrom ? ` · from ${campaign.sendFrom}` : ""}
      </p>
      <p style={{ margin: "0.25rem 0" }}>
        {campaign.totalRecipients} recipient
        {campaign.totalRecipients === 1 ? "" : "s"} ·{" "}
        <strong>{campaign.sent}</strong> sent ·{" "}
        <strong>{campaign.replied}</strong> replied
      </p>
      ...
      {campaign.status === "active" && (
        <p style={{ marginTop: "0.75rem", color: "#475569" }}>
          ...
          <strong>{dailyLimitRemaining}</strong> remaining under the daily
          limit.
        </p>
      )}
      ...
      <section aria-label="Message sequence" style={{ marginTop: "1.5rem" }}>
          ...
          {sequence.map((m, i) => (
            <li key={i} style={{ marginBottom: "0.5rem" }}>
              ...
              <span style={{ color: "#777", fontSize: "0.85rem" }}>
                ...
                {i === 0
                  ? "sent first"
                  : `${campaign.steps[i - 1]!.delayDays} days after the previous`}
              </span>
        ...
        <p style={{ color: "#777", fontSize: "0.85rem", margin: "0.25rem 0" }}>
          Personalize with merge variables: {"{{firstName}}"}, {"{{lastName}}"},{" "}
      ...
      <section aria-label="Recipients" style={{ marginTop: "1.5rem" }}>
        ...
        {recipients.length === 0 ? (
          <p style={{ margin: "0.25rem 0" }}>
            No recipients yet. Add them from a search or by contact id when you
            create the campaign (draft campaigns can be extended via the API).
          </p>
        ) : (
          <table>
            <thead>
              ...
              {recipients.map((r) => (
                <tr key={r.id}>
                  <td>
                    ...
                    {r.company ? (
                      <span style={{ color: "#777" }}> · {r.company}</span>
                    ) : null}
                  </td>
                  <td>
                    {r.status}
                    ...
                    {r.scheduledAt ? (
                      <div style={{ color: "#777", fontSize: "0.85rem" }}>
                        next {utcDay(r.scheduledAt)} (
                        {relativeDayLabel(r.scheduledAt, now)})
                      </div>
                    ) : null}
                  </td>
                  <td>
