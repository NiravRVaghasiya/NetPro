...
beforeEach(async () => {
  ...
  await fixture.conn.db.insert(fixture.conn.schema.contacts).values([
    { id: 'a', fullName: 'Ada Lovelace', company: 'Acme', relationshipScore: 0.9, lastInteraction: iso(5), source: 'test', createdAt: iso(300), updatedAt: NOW.toISOString() },
    { id: 'b', fullName: 'Bob', company: 'Acme', relationshipScore: 0.6, source: 'test', createdAt: iso(300), updatedAt: NOW.toISOString() },
    { id: 'z', fullName: 'Zoe Target', company: 'Beta', role: 'CTO', relationshipScore: 0.1, source: 'test', createdAt: iso(300), updatedAt: NOW.toISOString() },
  ]);
...
describe('/graph page', () => {
  ...
  it('includes a pending nudge in the empty state', async () => {
    fixture.sqlite.exec('DELETE FROM edges;');
    await fixture.conn.db.insert(fixture.conn.schema.edges).values({
      id: 'p1', sourceId: 'a', targetId: 'z', relation: 'mutual_network', strength: 0.3, confidence: 0.5,
      status: 'pending', bidirectional: true, source: 'linkedin_csv', discoveredAt: iso(2), updatedAt: iso(2),
    });
    const html = await render();
