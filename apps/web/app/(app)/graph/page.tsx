...
function PathChain({
  ...
  return (
    ...
    >
      <p style={{ margin: 0 }}>
        <strong>#{path.rank}</strong> · {path.hops} hop
        {path.hops === 1 ? "" : "s"} · chain strength{" "}
          ...
          <span key={n.contactId}>
            {i > 0 ? (
              <span style={{ color: "#9ca3af" }}>
                ...
                {n.via && n.via.minConfidence < 1
                  ? ` · conf ${n.via.minConfidence.toFixed(2)}`
                  : ""}
            ...
            <span
              ...
              title="your tie to this contact"
            >
              ({scoreLabel(n.relationshipScore)}
              {n.lastInteraction
                ? ` · ${n.lastInteraction.slice(0, 10)}`
                : " · no touch"}
              )
            </span>
          </span>
...
export default async function GraphPage({
  ...
  const [picklist, plan, planError] = await (async () => {
    ...
    if (!target)
      return [contacts, null as IntroPathPlan | null, null as string | null];
    try {
  ...
  const overview = target
    ? null
    : await getNetworkGraph(conn, { ...analysis, scope });
      ...
      {planError ? (
        ...
        <section>
          ...
          <p style={{ color: "#6b7280", marginTop: "0.25rem" }}>
            From{" "}
            <Link href={`/graph/${plan.origin.contactId}`}>
...
function Overview({
  ...
  graph: NonNullable<Awaited<ReturnType<typeof getNetworkGraph>>>;
}) {
  if (graph.degraded) {
    return <p style={{ color: "#b45309" }}>{graph.degraded.reason}</p>;
  }
  if (graph.nodes === 0) {
    return (
      <p style={{ color: "#9ca3af" }}>
        No confirmed edges yet — the pathfinder needs a graph to walk.{" "}
        <Link href="/import">Import connections</Link> (mutuals arrive as
        pending candidates) or <Link href="/edges">link two people</Link>.
        {graph.pendingCandidates > 0 ? (
          <>
            {" "}
            <Link href="/edges?status=pending">
              {graph.pendingCandidates} pending candidate
              {graph.pendingCandidates === 1 ? "" : "s"} waiting for you.
            </Link>
          </>
        ) : null}
      </p>
  ...
  return (
    <section style={{ marginTop: "1.5rem" }}>
      ...
      <p style={{ color: "#6b7280" }}>
        ...
        {graph.components.count === 1 ? "" : "s"} · modularity{" "}
      ...
      <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 280px" }}>
          ...
          <ul>
            {graph.centrality.top.map((t) => (
              <li key={t.contactId}>
                <Link href={`/graph/${t.contactId}`}>{t.fullName}</Link> —{" "}
                {t.degree} edge
                {t.degree === 1 ? "" : "s"}{" "}
                <Link
                  href={`/graph?target=${t.contactId}`}
                  style={{ color: "#6b7280" }}
                >
        ...
        <div style={{ flex: "1 1 280px" }}>
          ...
          {graph.warmIntros.length === 0 ? (
            <p style={{ color: "#9ca3af" }}>
              Every linked contact already reaches every hub directly.
            </p>
          ) : (
            <ul>
              {graph.warmIntros.map((w) => (
                <li key={`${w.contactId}-${w.targetId}`}>
                  <Link
                    href={`/graph?target=${w.targetId}&from=${w.contactId}`}
                  >
