...
export default async function GraphContactPage({
  params,
}: {
  ...
  let pos;
  try {
    pos = await getContactGraphPosition(conn, contactId, { scope });
  } catch (e) {
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: "1.5rem", fontWeight: 600 }}>
            ...
            {pos.centrality.degreeRank ? (
              ...
              >
                {" "}
                · rank #{pos.centrality.degreeRank} of {pos.analyzed.nodes}
              </span>
            ) : null}
          </div>
        </div>
        ...
        >
          ...
          <div style={{ fontSize: "1.5rem", fontWeight: 600 }}>
            {pos.reachableWithinDepth}
          </div>
          <div style={{ fontSize: "0.75rem", color: "#9ca3af" }}>
            contacts within 4 hops
          </div>
        </div>
        <div
          style={{
        ...
        >
          ...
          <div style={{ fontSize: "1.5rem", fontWeight: 600 }}>
            {scoreLabel(pos.contact.relationshipScore)}
          </div>
          <div style={{ fontSize: "0.75rem", color: "#9ca3af" }}>
            {pos.contact.lastInteraction
              ? `last touch ${utcDay(pos.contact.lastInteraction)}`
              : "no recorded touch"}
          </div>
        </div>
      </div>
      ...
      {pos.neighbors.length === 0 ? (
        <p style={{ color: "#9ca3af" }}>
          No edges from or to this contact —{" "}
          <Link href={`/edges`}>add one</Link> and the pathfinder can use it.
        </p>
      ) : (
        <table>
          <thead>
            ...
            {pos.neighbors.map((n) => (
              <tr key={n.edgeId}>
                <td>
                  ...
                  <Link href={`/contacts/${n.contactId}`}>{n.fullName}</Link>
                </td>
                <td>{n.relation}</td>
                <td
                  style={{
                    color: n.status === "pending" ? "#b45309" : undefined,
                  }}
                >
                  {n.status}
                </td>
      ...
      {pos.warmIntros.length > 0 ? (
        <section style={{ marginTop: "1.5rem" }}>
          <h2>
            Warm-intro suggestions involving{" "}
            {pos.contact.fullName.split(" ")[0]}
          </h2>
          <ul>
