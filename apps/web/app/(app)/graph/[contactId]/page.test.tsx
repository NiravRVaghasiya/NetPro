...
async function render(id: string): Promise<string> {
  const el = await GraphContactPage({ params: Promise.resolve({ contactId: id }) });
...
beforeEach(async () => {
  ...
  await fixture.conn.db.insert(fixture.conn.schema.contacts).values([
    { id: 'a', fullName: 'Ada', company: 'Acme', relationshipScore: 0.9, lastInteraction: iso(5), source: 'test', createdAt: iso(300), updatedAt: NOW.toISOString() },
    { id: 'b', fullName: 'Bob', company: 'Acme', relationshipScore: 0.6, source: 'test', createdAt: iso(300), updatedAt: NOW.toISOString() },
    { id: 'z', fullName: 'Zoe', company: 'Beta', relationshipScore: 0.2, source: 'test', createdAt: iso(300), updatedAt: NOW.toISOString() },
  ]);
  await fixture.conn.db.insert(fixture.conn.schema.edges).values([
    ...
    { id: 'p1', sourceId: 'b', targetId: 'z', relation: 'mutual_network', strength: 0.4, confidence: 0.5, status: 'pending', bidirectional: true, source: 'linkedin_csv', discoveredAt: iso(2), updatedAt: iso(2) },
  ]);
});
