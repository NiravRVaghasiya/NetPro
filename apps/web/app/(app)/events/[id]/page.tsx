...
import { getEvent, resolveEventRef, type EventDetail } from '@netpro/core/src/events';
  ...
  if (detail.attendees.length === 0) {
    return (
      <p style={{ color: '#9ca3af' }}>
        Nobody in your network is linked to this event yet — add someone below, or import an attendee
        list.
      </p>
    );
  }
      ...
      <tbody>
        {detail.attendees.map((a) => (
          <tr key={a.contactId} style={{ borderBottom: '1px solid #f3f4f6' }}>
            ...
            <td style={{ padding: '0.4rem 0.5rem', color: '#6b7280' }}>{a.company ?? '—'}</td>
            <td style={{ padding: '0.4rem 0.5rem', color: '#374151' }}>
              {[a.eventRole, a.attended ? 'attended' : 'planned'].filter(Boolean).join(' · ')}
            </td>
            <td style={{ padding: '0.4rem 0.5rem' }}>{scoreLabel(a.relationshipScore)}</td>
            <td style={{ padding: '0.4rem 0.5rem' }}>
              <RemoveAttendeeButton eventId={detail.event.id} contactId={a.contactId} />
            </td>
          </tr>
        ))}
      </tbody>
...
function Unmatched({ detail }: { detail: EventDetail }) {
  ...
  return (
    <section style={{ marginTop: '1.5rem' }}>
      <h2 style={{ fontSize: '1rem' }}>Unmatched attendees ({detail.unmatched.length})</h2>
      <p style={{ color: '#6b7280', margin: '0 0 0.5rem' }}>
        These lines from the imported list matched no contact — or matched more than one. Nothing was
        linked; name the right person to link them.
      </p>
...
export default async function EventDetailPage({
  ...
  const event = await resolveEventRef(conn, id).catch(() => null);
  if (!event) notFound();
  ...
  return (
    <div>
      ...
      <p style={{ margin: '0.25rem 0', color: '#475569' }}>
        {[when(detail, now), detail.event.location].filter(Boolean).join(' · ')} · source{' '}
        {detail.event.source}
      </p>
      ...
      <p style={{ margin: '0.5rem 0' }}>
        ...
        {detail.attendees[0] ? (
          <Link href={`/graph?target=${encodeURIComponent(detail.attendees[0]!.contactId)}`}>
            Find a warm intro to {detail.attendees[0]!.fullName}
          </Link>
        ) : (
          <>
            Once someone is linked, <Link href="/graph">Graph</Link> can plan the introduction.
          </>
        )}
      </p>
