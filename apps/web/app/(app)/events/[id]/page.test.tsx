...
vi.mock("../panels", () => ({
  ...
  LinkUnmatchedForm: ({ label }: { label: string }) => (
    <div data-testid="link-form">{label}</div>
  ),
}));
...
beforeEach(async () => {
  ...
  const { importEvents, resolveEventRef } =
    await import("@netpro/core/src/events");
...
describe("/events/[id] page", () => {
  ...
  it("has an empty state and never lists a soft-deleted contact", async () => {
    ...
    expect(html).toContain(
      "Nobody in your network is linked to this event yet",
    );
