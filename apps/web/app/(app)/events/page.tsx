  ...
  return (
    <section style={{ marginTop: "1.75rem" }}>
      ...
      <p style={{ color: "#6b7280" }}>
        Ranked 0.6 × how many of your contacts went, 0.2 × how much of your
        network shares their industries, 0.2 × timing. Every score is explained
        underneath.
      </p>
      <ol style={{ paddingLeft: "1.25rem" }}>
        {rows.map((r) => (
          <li key={r.event.id} style={{ marginBottom: "0.6rem" }}>
            <Link href={`/events/${encodeURIComponent(r.event.id)}`}>
              {r.event.name}
            </Link>{" "}
            <strong>{r.score.toFixed(2)}</strong> ·{" "}
            {when(r.event.startsAt, r.event.endsAt, now)}
  ...
  if (events.length === 0) {
    return (
      <p style={{ color: "#9ca3af" }}>
        No events yet. Add one below, or import a CSV of events and attendees —
        NetPro matches the attendee list against your contacts for you.
      </p>
    );
  }
  return (
...
export default async function EventsPage({
  ...
  const [{ events, total }, recommendations, status] = await Promise.all([
    listEvents(conn, { ...eventListParams(sp), limit: 100, scope }),
    recommendEvents(conn, { limit: 5, now, scope }),
    eventsStatus(conn, scope),
  ]);
  ...
  return (
    <div>
      ...
      <p style={{ color: "#475569" }}>
        Who in your network was at a conference, and which one you should go to
        next. Import an attendee list and NetPro matches it against your
        contacts — by email, then by name, never by guessing.{" "}
        <Link href="/graph">Warm intros</Link> ·{" "}
        <Link href="/edges">Edges</Link>
      </p>
      <p style={{ color: "#6b7280" }}>
        {status.events} event{status.events === 1 ? "" : "s"} · {status.links}{" "}
        attendance row
        {status.links === 1 ? "" : "s"} · {status.attendees} of{" "}
        {status.contacts} contacts have been somewhere.
      </p>
