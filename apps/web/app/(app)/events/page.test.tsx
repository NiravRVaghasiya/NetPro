...
function tableOf(html: string): string {
  return html.match(/<table[^>]*data-testid="events-table"[\s\S]*?<\/table>/)?.[0] ?? '';
}
...
describe('/events page', () => {
  it('renders the list, the counts and the panels', async () => {
    ...
    expect(html).toContain('3 events · 3 attendance rows · 2 of 2 contacts have been somewhere');
  ...
  it('links each event to its detail page and shows network overlap', async () => {
    const html = await render();
    const id = fixture.sqlite.prepare("SELECT id FROM events WHERE name = 'React Conf'").get() as { id: string };
