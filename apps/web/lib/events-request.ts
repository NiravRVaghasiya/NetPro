...
export const boundedName = (raw: unknown, field: string): string | null =>
  boundedText(raw, field, 200);
...
export function eventListParams(sp: URLSearchParams): {
  ...
  return {
    ...
    upcoming:
      upcoming === null ? undefined : upcoming === "1" || upcoming === "true",
...
export async function resolveOptionalEvent(
  ...
  if (trimmed.length > 320)
    throw new CrmRequestError(400, "event selector is too long.");
  try {
    return await resolveEventRef(conn, trimmed, scope);
  } catch (e) {
...
export async function readEventCsv(request: Request): Promise<string> {
  ...
  if (file.size > MAX_EVENT_CSV_BYTES) {
    throw new CrmRequestError(
      413,
      `CSV must be ${MAX_EVENT_CSV_BYTES / 1024} KiB or smaller.`,
    );
  }
  return file.text();
}
