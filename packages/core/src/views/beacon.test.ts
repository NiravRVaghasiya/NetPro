...
describe('viewed page allowlist (v2.5 phase 2)', () => {
  it('exposes the plan-mandated set and snaps unknown values to /card', () => {
    expect(VIEWED_PAGES).toEqual(['/card', '/card/vcard', 'blog', 'portfolio', 'embed']);
...
describe('header extraction (v2.5 phase 2)', () => {
  ...
  it('falls back to X-Real-Ip and accepts IPv6', () => {
    expect(extractViewerIp(headers({ 'x-real-ip': '198.51.100.23' }))).toBe('198.51.100.23');
    expect(
      extractViewerIp(headers({ 'x-forwarded-for': '2001:db8::1' })),
    ).toBe('2001:db8::1');
  ...
  it('reads geo from Vercel or Cloudflare headers, capped and trimmed', () => {
    ...
    expect(extractViewerGeo(headers({ 'x-vercel-ip-country': 'x'.repeat(100) }))).toEqual({
      country: 'x'.repeat(64),
      city: null,
    });
...
describe('dedup decision (v2.5 phase 2)', () => {
  const row = (fingerprint: string | null, viewerIp: string | null, page: string, viewedAt: string) => ({
  ...
  it('skips the same fingerprint within 5 minutes', () => {
    const verdict = shouldCountView({
      ...
      viewedPage: '/card',
      now: NOW,
      recent: [row('f'.repeat(16), 'a'.repeat(16), '/card', '2026-09-08T09:58:00.000Z')],
    });
  ...
  it('allows the same fingerprint once the 5-minute window has passed', () => {
    ...
    const verdict = shouldCountView({
      ...
      viewedPage: 'blog',
      now: NOW,
      recent: [row('f'.repeat(16), 'a'.repeat(16), '/card', '2026-09-08T09:54:00.000Z')],
    });
  ...
  it('skips the same IP hash + page within 1 hour even with a different fingerprint', () => {
    const verdict = shouldCountView({
      ...
      viewedPage: '/card',
      now: NOW,
      recent: [row('f'.repeat(16), 'a'.repeat(16), '/card', '2026-09-08T09:30:00.000Z')],
    });
  ...
  it('honours configurable windows and ignores future-dated junk rows', () => {
    const verdict = shouldCountView({
      ...
      viewedPage: '/card',
      now: NOW,
      recent: [row('f'.repeat(16), 'a'.repeat(16), '/card', '2026-09-09T10:00:00.000Z')],
...
describe('recordView — the profile_views producer (v2.5 phase 2)', () => {
  ...
  it('records a hardened row: salted hashes, no raw IP anywhere', async () => {
    ...
    expect(result).toMatchObject({ counted: true, reason: 'new', isBot: false, isOwnerView: false });

    const stored = singleRow(
      sqlite.prepare('SELECT * FROM profile_views').all() as Array<Record<string, unknown>>,
    );
  ...
  it('labels bots and owner sessions without refusing the view', async () => {
    ...
    expect(bot).toMatchObject({ counted: true, isBot: true, isOwnerView: false });
  ...
  it('dedups rapid re-sends of the same view with an explanatory reason', async () => {
    ...
    const reload = await recordView(conn, { ...base, now: new Date(NOW.getTime() + 2 * 60_000) });
    ...
    const blog = await recordView(conn, { ...base, viewedPage: 'blog', ip: '198.51.100.50' });
    expect(blog.counted).toBe(true);
    const otherIp = await recordView(conn, { ...base, ip: '198.51.100.51' });
    expect(otherIp.counted).toBe(true);

    expect(
      (sqlite.prepare('SELECT count(*) AS n FROM profile_views').get() as { n: number }).n,
    ).toBe(4);
  });
  ...
  it('clamps duration to a sane bound and normalizes the page', async () => {
    await recordView(conn, { ...base, durationMs: VIEW_DURATION_MAX_MS + 1, viewedPage: 'x/../etc' });
    const stored = singleRow(
      sqlite.prepare('SELECT duration_ms, viewed_page FROM profile_views').all() as Array<{
...
describe('signed ?v= contact-resolution tokens (v2.5 phase 2)', () => {
  ...
  it('round-trips a freshly minted token', () => {
    ...
    expect(resolveContactFromToken(token, { secret: SECRET, now: NOW })).toBe(CONTACT_ID);
  });

  it('expires and rejects wrong secrets, tampering, and garbage', () => {
    const token = createContactViewToken(CONTACT_ID, SECRET, NOW);
    expect(
      resolveContactFromToken(token, { secret: SECRET, now: new Date(NOW.getTime() + VIEW_TOKEN_TTL_MS + 1000) }),
    ).toBeNull();
    expect(resolveContactFromToken(token, { secret: 'other-secret', now: NOW })).toBeNull();
    const [payload] = token!.split('');
    const tampered =
      (payload === 'A' ? 'B' : 'A') + token!.slice(1);
    expect(resolveContactFromToken(tampered, { secret: SECRET, now: NOW })).toBeNull();
    expect(resolveContactFromToken('not-a-token', { secret: SECRET, now: NOW })).toBeNull();
    expect(resolveContactFromToken('', { secret: SECRET, now: NOW })).toBeNull();
    expect(resolveContactFromToken(null, { secret: SECRET, now: NOW })).toBeNull();
    expect(resolveContactFromToken(token, { secret: '', now: NOW })).toBeNull();
  });

  it('refuses to mint for bad contact ids or oversized TTLs', () => {
    expect(createContactViewToken('bad id with spaces', SECRET, NOW)).toBeNull();
  ...
  it('rejects hand-forged tokens whose expiry exceeds the 30-day cap', () => {
    ...
    const forged = Buffer.from(`${CONTACT_ID}|${farFuture}|${hmac}`, 'utf8')
      ...
      .replace(/\//g, '_')
      .replace(/=+$/, '');
    expect(resolveContactFromToken(forged, { secret: SECRET, now: NOW })).toBeNull();
  });
});
...
describe('findLiveContactId (v2.5 phase 2)', () => {
  ...
  const addContact = (id: string, deletedAt: string | null = null) => {
    sqlite
      ...
      .run(id, 'Ada Lovelace', 'csv', NOW.toISOString(), NOW.toISOString(), deletedAt);
  };
