...
async function findEventByName(
  ...
  if (conn.dialect === 'sqlite') {
    const rows = await conn.db
      ...
      .where(eq(conn.schema.events.name, name))
      .limit(1);
...
export async function findOrCreateEvent(
  conn: SqliteConn | PgConn,
  input: { name: string; location?: string | null; startsAt?: string | null; source?: string },
  opts: GraphOptions = {}
): Promise<EventRow> {
  ...
  const existing = await findEventByName(conn, name);
  if (existing) return existing;
  const now = resolveNow(opts);
  const row: EventRow = {
    id: randomUUID(),
    name,
    location: optionalText(input.location, GRAPH_LIMITS.eventLocation, 'location') ?? null,
...
export async function recordEventAttendance(
  conn: SqliteConn | PgConn,
  input: { contactId: string; eventName: string; location?: string | null; role?: string | null },
  opts: GraphOptions = {}
): Promise<{ event: EventRow; linked: number }> {
  const contact = await getContactById(conn, input.contactId);
  if (!contact) {
    throw new GraphError('not_found', `No contact with id "${input.contactId}".`);
  }
  ...
  const others = (await listEventAttendeeIds(conn, event.id)).filter((id) => id !== contact.id);
  let linked = 0;
