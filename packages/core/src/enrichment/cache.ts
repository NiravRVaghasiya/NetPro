...
export async function getCached(
  ...
  if (conn.dialect === "sqlite") {
    ...
    const rows = await conn.db
      ...
      .where(
        and(
          eq(e.contactId, contactId),
          eq(e.provider, provider),
...
export async function setCached(
  ...
  const workspaceId = resolveScope(scope).workspaceId;
  ...
  if (conn.dialect === "sqlite") {
    const payload = serializePayload(conn.dialect, {
      data: result.data,
      raw: result.rawPayload,
    });
    await conn.db
      .insert(conn.schema.enrichments)
      .values({ ...values, rawPayload: payload });
    return;
  }
