  ...
  try {
    for (let i = 0; i < rows.length; i += chunk) {
      const values = rows
        ...
        .map(
          (r) =>
            `(${r.map((v) => (v === null ? "NULL" : typeof v === "number" ? String(v) : `'${v}'`)).join(",")})`,
        )
        .join(",");
      sqlite
        .prepare(
          `INSERT INTO ${tableFor(columns)} (${columns}) VALUES ${values}`,
        )
        .run();
    }
  } finally {
...
beforeAll(() => {
  ...
  const contactCols =
    "id,workspace_id,full_name,source,company,industry,role,relationship_score,last_interaction,created_at,updated_at";
  ...
  const edgeCols =
    "id,workspace_id,source_id,target_id,relation,strength,bidirectional,source,confidence,status,discovered_at,updated_at";
  ...
  const viewCols =
    "id,workspace_id,viewer_ip,referrer,resolved_contact,viewer_fingerprint,is_bot,is_owner_view,session_id,duration_ms,viewed_page,viewed_at,country";
  ...
  const itemCols =
    "id,workspace_id,url,url_norm,title,platform,type,published_at,author,tags,summary,source,created_at,updated_at";
  ...
  const metricCols =
    "id,workspace_id,content_id,fetched_at,source,views,likes,raw_payload,created_at";
...
describe("v2.5 Phase 6 — SQLite performance budget (10k views / 1k content / 5k metrics)", () => {
  it("fixture size sanity", () => {
    expect(
      sqlite.prepare("SELECT COUNT(*) n FROM profile_views").get() as {
        n: number;
      },
    ).toEqual({
      n: VIEWS,
    ...
    expect(
      sqlite.prepare("SELECT COUNT(*) n FROM content_items").get() as {
        n: number;
      },
    ).toEqual({
      n: CONTENT_ITEMS,
    ...
    expect(
      sqlite.prepare("SELECT COUNT(*) n FROM content_metrics").get() as {
        n: number;
      },
    ).toEqual({
      n: METRICS,
  ...
  it("dashboard GET /api/analytics (getNetworkOverview) under 500 ms", async () => {
    ...
    const { value, ms } = await timed(() =>
      getNetworkOverview(conn, { now: NOW }),
    );
    console.info(
      `[perf] dashboard GET /api/analytics: ${ms.toFixed(1)} ms (budget 500 ms)`,
    );
  ...
  it("views GET /api/card/views (getViewsOverview) under 100 ms", async () => {
    ...
    const { value, ms } = await timed(() =>
      getViewsOverview(conn, { days: 30, now: NOW }),
    );
    console.info(
      `[perf] views GET /api/card/views: ${ms.toFixed(1)} ms (budget 100 ms)`,
    );
    ...
    expect(
      value.stats.excluded.bots + value.stats.excluded.ownerViews,
    ).toBeGreaterThan(0);
  ...
  it("content GET /api/content (listContentSummaries) under 100 ms", async () => {
    ...
    const { value, ms } = await timed(() =>
      listContentSummaries(conn, { limit: 50, now: NOW }),
    );
    console.info(
      `[perf] content GET /api/content: ${ms.toFixed(1)} ms (budget 100 ms)`,
    );
  ...
  it("records the dashboard content path (getContentOverview) for the progress doc", async () => {
    ...
    const { value, ms } = await timed(() =>
      getContentOverview(conn, { now: NOW }),
    );
    console.info(
      `[perf] content overview (dashboard strip): ${ms.toFixed(1)} ms (recorded)`,
    );
