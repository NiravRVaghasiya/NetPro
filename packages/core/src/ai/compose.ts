...
import { buildOutreachMessages, TONE_VALUES, type ComposeOutreachInput, type OutreachTone } from './prompt';
 ...
 */
export function validateComposeInput(input: ComposeOutreachInput): ComposeOutreachInput {
  ...
  if (input.context && input.context.length > LIMITS.context) {
    throw new AiProviderError('invalid_input', `Context must be ${LIMITS.context} characters or fewer.`);
  }
  if (input.purpose && input.purpose.length > LIMITS.purpose) {
    throw new AiProviderError('invalid_input', `Purpose must be ${LIMITS.purpose} characters or fewer.`);
  }
  if (r.name && r.name.length > LIMITS.name) {
    throw new AiProviderError('invalid_input', `Recipient name must be ${LIMITS.name} characters or fewer.`);
  }
  if (r.company && r.company.length > LIMITS.company) {
    throw new AiProviderError('invalid_input', `Company must be ${LIMITS.company} characters or fewer.`);
  }
  if (r.role && r.role.length > LIMITS.role) {
    throw new AiProviderError('invalid_input', `Role must be ${LIMITS.role} characters or fewer.`);
  }
  if (r.email && r.email.trim() && !EMAIL_SHAPE.test(r.email.trim())) {
    throw new AiProviderError('invalid_input', `"${r.email}" is not a valid email address.`);
  }
...
export async function composeOutreachMessage(
  ...
  const raw = await deps.provider.complete(messages, { model: normalized.model });
