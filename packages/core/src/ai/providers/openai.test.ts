...
import { createOpenAiProvider, OPENAI_DEFAULT_BASE_URL, OPENAI_DEFAULT_MODEL } from './openai';
import { AiProviderError } from '../types';

function fakeFetch(
  response: Partial<Response> | ((url: string, init?: RequestInit) => Partial<Response>),
...
describe('createOpenAiProvider', () => {
  ...
  it('uses the default model and a baseUrl override', async () => {
    ...
    const provider = createOpenAiProvider(
      { apiKey: 'k', baseUrl: 'https://gateway.example.com/v1/', model: 'gpt-4o' },
      fetchImpl,
    );
  ...
  it('throws an upstream_error with status and provider message on failure', async () => {
    ...
    await expect(provider.complete([{ role: 'user', content: 'x' }])).rejects.toThrow(
      /OpenAI request failed: 401 bad api key/,
    );
  });
