...
export const TONE_VALUES = ['professional', 'warm', 'casual', 'friendly'] as const;
 ...
 */
export function buildOutreachMessages(input: ComposeOutreachInput): ChatMessage[] {
  ...
  if (facts.length > 0) {
    ...
    lines.push('- (No profile details are available — keep the greeting generic and do not assume any facts.)');
  }

  lines.push('');
  lines.push(`- Tone: ${input.tone ?? 'professional'}`);
  if (input.senderName) lines.push(`- Sign off as: ${input.senderName}`);
  if (input.context) lines.push(`- Context for why I'm reaching out: ${input.context}`);
