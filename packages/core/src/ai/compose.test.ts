...
describe('composeOutreachMessage', () => {
  it('builds a draft from the provider reply with metadata', async () => {
    const { provider, calls } = fakeProvider('{"subject":"Hi Jane","body":"Great talk at React Conf."}');
    const draft = await composeOutreachMessage(
      { recipient: { name: 'Jane', company: 'Stripe' }, tone: 'warm', senderName: 'Alex' },
      { provider, now: NOW },
    );
  ...
  it('defaults the tone to professional', async () => {
    ...
    const draft = await composeOutreachMessage({ recipient: { name: 'Jane' } }, { provider, now: NOW });
  ...
  it('propagates upstream errors', async () => {
    const provider: AiProvider = {
      ...
      async complete() {
        throw new AiProviderError('upstream_error', 'Anthropic request failed: 500');
      },
    };
    await expect(
...
describe('validateComposeInput', () => {
  it('requires a name or email', () => {
    expect(() => validateComposeInput({ recipient: {} })).toThrowError(AiProviderError);
    try {
    ...
    expect(validateComposeInput({ recipient: { email: 'a@b.com' } }).tone).toBe('professional');
  });

  it('rejects unknown tones', () => {
    try {
      validateComposeInput({ recipient: { name: 'X' }, tone: 'aggressive' as never });
      throw new Error('expected');
    } catch (e) {
      expect((e as AiProviderError).code).toBe('invalid_input');
      expect((e as Error).message).toMatch(/professional, warm, casual, friendly/);
    }
  });

  it('enforces length caps', () => {
    expect(() =>
      validateComposeInput({ recipient: { name: 'X' }, context: 'a'.repeat(2001) }),
    ).toThrow(/Context must be 2000/);
    expect(() =>
      validateComposeInput({ recipient: { name: 'X' }, purpose: 'a'.repeat(501) }),
  ...
  it('validates email shape', () => {
    expect(() => validateComposeInput({ recipient: { email: 'not-an-email' } })).toThrow(
      /not a valid email/,
    );
  });
});
