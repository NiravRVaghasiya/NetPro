...
The `enrichment`, `importPipeline`, and `exportPipeline` namespaces now resolve to the real modules built in Tasks 1-8 (their `index.ts` files replace the scaffold's one-line `MODULE_NAME` placeholders). `search`, `analytics`, `ai`, and `crm` are untouched — still scaffold placeholders, out of scope for this phase. (An earlier version of this step accidentally dropped `analytics` from the barrel entirely — caught during Task 8's implementation and fixed here; if you're implementing from a stale copy of this plan, make sure `analytics` is present.)
