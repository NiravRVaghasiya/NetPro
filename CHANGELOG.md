...
- Core `plugins` module (`packages/core/src/plugins/`):
  - `types.ts`: `PluginManifest`, `SettingSpec`, `PluginCapability` (source|enricher|ai-provider|content-provider|event-discovery|command), `Plugin`, `PluginApi` (injection: fetch wrapper enforcing host allowlist, workspace-scoped data API, settings reader from `plugin_settings` + secrets from vault `plugin.<name>.<key>`, structured rate-capped logger into `activity_log`, capability registrars taking existing interfaces).
  - `manifest.ts`: strict validation — npm-style name regex, semver version, engine range syntax (^ ~ >= > = exact, OR groups), capabilities allowlist, network host allowlist (exact-host with explicit `*.` wildcard, no accidental subdomain, max 100 hosts, size guard 20 KiB, max 100 settings, duplicate key check, entry path traversal guard), `satisfiesEngineRange` with ^ ~ >= > logic, `definePlugin` helper.
  - `repository.ts`: workspace-scoped CRUD — `listPlugins`, `getPluginByName`, `createPlugin` (conflict on dup), `updatePluginEnabled`, `updatePluginSettings`, `deletePlugin`; JSON handling portable (stringified on Postgres, object on SQLite), `enabled` bool conversion.
  - `runtime.ts`: loader & lifecycle — `NETPRO_PLUGIN_DIR` (default `./plugins`), `discoverPluginsFromDir` reads each subdir manifest.json + entry (ESM `import()` via file URL, no `require` of arbitrary URLs), engine range check refuses incompatible with self-explaining error, `createFetchWrapper` blocks undeclared hosts and audits `plugin.fetch_blocked`, `createPluginApi` with settings/secret/log/registrars, in-memory per-workspace registry (`enrichers`, `aiProviders`, `contentProviders`, `eventDiscovery`, `commands`), `loadPluginsForWorkspace` clears and re-registers enabled plugins, crashing plugin isolated (wraps register in boundary, logs `plugin.load_error`, disables itself, never takes app down), `enablePlugin`/`disablePlugin` audited (`plugin.enabled`/`disabled`/`removed`) and registry updated.
- CLI: `netpro plugin` (20th top-level command) — `list` (enabled/disabled + source), `paths` (load roots), `discover` (disk scan with errors), `info <name>`, `install <manifestPath>` (local file, Phase 5; marketplace in Phase 6, disabled by default), `enable <name> --i-have-reviewed-permissions` (shows capabilities + network if not reviewed, permission review gate), `disable`, `rm`, `settings <name> <json>` (non-secret JSON). Uses `openDb` + `resolveCliScope` pattern, workspace-scoped.
- Web: admin-only `/settings/plugins` page with permission manifest display, enable/disable with review gate, remove, settings JSON editor, secret hint (stored in vault via `/settings/keys` as `plugin.<name>.<key>`), plus APIs `GET /api/plugins`, `GET/DELETE /api/plugins/[name]`, `POST /api/plugins/[name]/enable` (requires `reviewed:true`), `/disable`, `/settings`. All admin+ role floor, `requireMembership('admin')`, audited. Trust model documented plainly: plugins run in-process with Node privileges, network boundary enforced, data boundary scoped, operator review required, no auto-install.
- Reference plugin `plugins/example-event-discovery/` — minimal offline event-discovery plugin exercising every contract piece: manifest with one network host `api.example.com`, settings (secret `api_key` + string `default_location`), `index.js` entry exporting `{ manifest, register }` that registers an `event-discovery` provider (canned results, no real network, demonstrates `api.fetch` would be blocked if host not allowed, `getSetting`, `getSecret`, `log`) and a `command` (`example`). Doubles as marketplace index entry #1.
- Tests: `plugins/manifest.test.ts` — malformed, bad semver, invalid engine, unknown capability, oversized settings, duplicate keys, invalid network hosts, path traversal, wildcard acceptance, engine range satisfaction (^ ~ >= OR). `plugins/runtime.test.ts` — fetch permission enforcement (blocked→audited, exact host pass, wildcard subdomain, evil.com blocked), data boundary (listPlugins workspace-scoped, scope guard), lifecycle (enable→registered, disable→unregistered, crashing plugin isolated & disabled & audited, engine mismatch self-explaining, discovery adversarial: no manifest ignored, entry throws reported). Migration test for `0013` in `migrations.test.ts`.
- Compatibility: single-workspace single-member behaves identically — no plugins installed, empty registry, no perf impact, existing provider seams unchanged.
